...
public class ManagePan {
	...
	private final JLabel speedSimulationLabel = new JLabel(
			"Pas en milliseconde :");
	private final JTextField speedSimulationTextField = new JTextField("100");
		...
		this.sizeMap.addKeyListener(new KeyListener() {
			...
			public void keyReleased(final KeyEvent arg0) {
				...
				if (content.matches("\\d{1,3}")
					...
					ManagePan.this.window.getContentPane().remove(
							ManagePan.this.window.getPanWest());
					ManagePan.this.window.setPanWest(ManagePan.this.window
							.getMapPan().initMapPan());

					ManagePan.this.window.getContentPane().add(
							ManagePan.this.window.getPanWest(),
							BorderLayout.WEST);
					ManagePan.this.window.getContentPane().validate();
